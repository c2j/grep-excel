...
TUI tool for searching tabular data files (Excel/CSV/HTML/text/Markdown) with DuckDB-powered performance.
...
| Tool | Description |
|------|-------------|
| `import_file` | Import a tabular file (Excel/CSV/HTML/text/Markdown) |
...
- `.docx` — Word documents (tables extracted from word/document.xml)
- `.pptx` — PowerPoint presentations (tables extracted from each slide)
...
基于 DuckDB 的多格式表格文件搜索 TUI 工具（Excel/CSV/HTML/文本/Markdown）。
...
- `.docx` — Word 文档（从 word/document.xml 提取表格）
- `.pptx` — PowerPoint 演示文稿（从每张幻灯片提取表格）
