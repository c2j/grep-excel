...
# 特定引擎测试（根是虚拟 manifest，带 feature 必须指定 -p）
cargo test -p grep-excel --features engine-duckdb
cargo test -p grep-excel --features engine-sqlite

# 全功能测试（含 MCP）
cargo test -p grep-excel --features full

# 提交前门禁（与 .github/workflows/ci.yml 逐条一致）
cargo fmt --all -- --check
cargo clippy -p grep-excel --features full -- -D warnings
...
> ⚠️ 裸 `cargo test`（default features）**覆盖不到 CI 跑的组合**。CI 用的是上面三条（`grep-excel-core` 带 pdf/parquet/archive、`grep-excel` 带 `full`，clippy 带 `-D warnings`）——只跑裸 `cargo test` 会出现「本地绿、CI 红」。

循环内只跑受影响 crate；提交前跑上面的完整门禁。
