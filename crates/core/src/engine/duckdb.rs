    ...
    fn import_excel(&mut self, path: &Path, progress: &dyn Fn(usize, usize)) -> Result<FileInfo> {
        #[cfg(feature = "archive-support")]
        {
            if let Some(format) = crate::archive::detect_archive(path) {
                return self.import_archive(path, format, progress);
            }
        }
                ...
                let is_import: bool = self
                    ...
                    .map(|mut stmt| {
                        stmt.query_map([], |row: &::duckdb::Row| row.get::<_, String>(0))
                            .ok()
                            .map(|mut mapped| {
                                mapped.any(|r: std::result::Result<String, ::duckdb::Error>| {
...
impl DuckDbEngine {
    ...
    fn import_archive(
        ...
        for entry in &table_entries {
            ...
            let result = self.import_excel_sheets_with_name(&tmp_path, &virtual_name, progress);
    ...
    fn import_excel_sheets_with_name(
        &mut self,
        path: &Path,
        display_name: &str,
        progress_callback: &dyn Fn(usize, usize),
