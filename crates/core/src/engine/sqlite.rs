...
impl SqliteEngine {
    ...
    fn import_archive_from_format(
        ...
        let mut all_sheets: Vec<(String, usize)> = Vec::new();
        let mut total_rows: usize = 0;

        for entry in &table_entries {
            let tmp_path = extract_entry(archive_path, &entry.path, format)?;
            let virtual_name = format!("{}::{}", archive_name, entry.path);
            let result = self.import_excel_file_with_name(&tmp_path, &virtual_name, progress);
            let _ = std::fs::remove_file(&tmp_path);
            let info = result?;
            all_sheets.extend(info.sheets);
    ...
    fn import_excel_file_with_name(
        &mut self,
        path: &Path,
        display_name: &str,
        progress: &dyn Fn(usize, usize),
    ...
    fn build_where_clause(query: &SearchQuery, col_names: &[String]) -> (String, Vec<String>) {
        ...
        for col in target_cols {
            match query.mode {
                ...
                SearchMode::Regex => {
                    or_parts.push(format!("regexp(?, {})", quote_ident(col)));
                    values.push(format!("(?i){}", query.text));
                }
            }
        }

        // Build AND parts from conditions (SQLite has no ILIKE, use UPPER)
        ...
        for cond in &query.conditions {
            ...
            let clause = match cond.operator.as_str() {
                "=" | "==" => format!("{} = ?", col),
                "!=" | "<>" => format!("{} <> ?", col),
                "ILIKE" => format!("UPPER({}) LIKE UPPER(?)", col),
            ...
            for (row_id, values) in matched_rows {
                ...
                let context = if query.context_lines.unwrap_or(0) > 0 {
                    ...
                    let ctx_rows: Vec<Vec<String>> = ctx_stmt
                        .query_map(rusqlite::params![row_id - n, row_id + n], |row| {
