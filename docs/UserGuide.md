...
### Q: 为什么 Excel 中的日期显示为 YYYYMMDD 格式？

A: Excel 内部将日期存储为序列号（如 `46188` = 2026-06-15）。grep-excel 会**自动检测日期列**并将序列号转换为 `YYYYMMDD` 字符串，因此：

- 日期列不再显示为 `46188`，而是显示 `20260615`
- 可以直接用日期片段搜索，如 `-q 0615` 匹配 6 月 15 日
- `--repair` 修复模式也会执行同样的日期转换

检测逻辑分两层：① 如果列中已有 `Data::DateTime` 类型单元格（高置信度）；② 如果列名包含日期相关关键词且超过 50% 的数值在 Excel 序列号范围内（兜底检测）。数值列（如薪资）不受影响。
